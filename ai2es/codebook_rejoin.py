...
def main(class_: str):
    ...
    for filename in files:
        if not find_file(filename, dest):
            shutil.copy(os.path.join(src, class_, filename), os.path.join(dest, class_, filename))

if __name__ == "__main__":
    Parallel(n_jobs = 3)(delayed(main)(class_) for class_ in ['no_precip', 'obstructed', 'precip'])